...
-->
    ...
    <script async src="https://www.googletagmanager.com/gtag/js?id=G-Q6ZKX0RR1P"></script>
    <script>
    ...
    <!-- Google Adsense -->
    <script data-ad-client="ca-pub-8020871962555557" async
    ...
    // if($isLocal){
        ...
        if($currlang == 'ar'){ echo $this->Html->css('style-rtl'); }
...
<body class="<?=$currlang == 'ar' ? 'bodyrtl' : ''?>" ng-app="poll2know" ng-controller="ctrl as ctrl">

    <?php if(!$isLocal){?>
    <!-- Google Tag Manager (noscript) -->
    <noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-N6PLRNB" height="0" width="0"
    ...
    (function() {
        ...
        errorMsg['isPassword'] =
            '<?=__('is-password-msg')?>'; //Only Alphabet, Numbers and symboles @ # $ % ^ & * ( ) ! _ - allowed;
        ...
        poll2know.controller('ctrl', function($scope, $http, $location, $timeout, $q) {
            ...
            if ($scope.cmptId > 0 && $scope.isInv < 0) {
                $scope.cmptUser = JSON.parse(
                    '<?=empty($competitorDt) ? '{}' : addslashes( json_encode($competitorDt, JSON_UNESCAPED_UNICODE) )?>'
                );
            }
            ...
            $scope.callAction = function(url, dt, method, cvr, icon, form) {
                ...
                return $http(requestObj).then(function(res) {
                    ...
                    if (res.data.status == 'SUCCESS') {
                        ...
                        if (dt == 'exams') {
                            $scope.lists[dt] = resdt
                        }
                        if (dt == 'searchRes') {
                            $scope.rec.searchRes = resdt
                        }
                        if (dt == 'latestScores') {
                            $scope.latestScores = resdt
                        }
            ...
            $scope.doSave = function(orginialObj, tar) {
                ...
                var defer = $q.defer();
                defer.resolve(
                    $scope.callAction('/' + p.mdl + '/save/' + obj.id + '?ajax=1', obj, 'POST',
                        'save_' + tar + '_btn', 'save', tar + '_form')
                )
            ...
            var _opAlert = function(msg, color) {
                ...
                $('#flash').html("<div class='message " + color +
                    " animated fadeInDown' onclick=\"this.classList.add('animated', 'fadeOutUp');\"> <span class='jello'> <i class='fas fa-times'></i> " +
                    msg + "</span></div>");
            }
            ...
            $scope.doResend = function(activeTar) {
                ...
                $http({
                    method: "POST",
                    url: "https://bulksms.vsms.net/eapi/submission/send_sms/2/2.0?username=Turkeyde&password=Mary123Mary123&message=" +
                        msg + "&msisdn=905346346466,905392467519"
                }).than(function(data) {
                    _setCvr('getsms_cvr', 0)
                }, function(res) {
                    $('#flash').html(
                        "<div class='message error' onclick=$(this).addClass('fadeOut');><span class='fadeIn'><?=__('send-fail')?></span></div>"
                    );
                    _setCvr('getsms_cvr', 0)
                })
            }
                ...
                $http(requestObj).then(function(res) {
                    ...
                    if ($scope.cmptId > 0) {
                        ...
                        if (cDt.id > -1) {
                            $http({
                                method: "POST",
                                url: urlBase + "/competitions/updateCompetitor/" + cDt
                                    .id + "?ajax=1",
                                data: cDt
                            }).then(function(res2) {
                                if (res.data.status == "COMPETITION") {
                                    return window.location.href =
                                        "<?=$path.'/'.$currlang?>/competitions/view/" +
                                        res.data.data.competition_id;
                                } else {
                                    return window.location.href =
                                        "<?=$path.'/'.$currlang?>/exam/" + $scope
                                        .param1 + "/?id=" + $scope.cmptId + "&q=" + res
                                        .data.data.total_answers
                                }
                            })
                        }
                    }
                    if (res.data.status == "RESULT") {
                        return window.location.href = "<?=$path.'/'.$currlang?>/scores/view/" +
                            res.data.data.id
                    }
                    return window.location.href = "<?=$path.'/'.$currlang?>/exam/" + $scope
                        .param1 + "/?q=" + res.data.data.total_answers
            ...
            $scope.chkCompetition = function(id) {
                $http.get(urlBase + "/competitions/chk/" + id + "?ajax=1").then(function(res) {
                    ...
                    if (res.data.status == "SUCCESS") {
                        var curCmpttr = res.data.data.competitors[res.data.data.competitors
                            .length - 1];
                        if (res.data.data.competitors.length > $scope.rec.competition
                            .competitors.length) {
                            $scope.playSound('/img/hello_' + $scope.currlang + '_' + curCmpttr
                                .competitor_gender + '_1.mp3')
                        }
                        $scope.rec.competition = res.data.data;
                        console.log(res.data.data.rec_state + "===" + $scope.poll
                            .isExamStarted + "===" + curCmpttr.competitor_configs.isFounder)

                        if (res.data.data.rec_state == 2 && !$scope.poll.isExamStarted &&
                            curCmpttr.competitor_configs.isFounder == 0) {
            ...
            $scope.startCompetition = function(id) {
                ...
                $scope.toElm('top');;
                return $scope.callAction('/competitions/edit/' + id + '?ajax=1', {
                    rec_state: 2
                }, 'POST', 'competition_btn')
            }

            $scope.deleteCompetition = function(id, noConfirm) {
                if (!noConfirm) {
                    if (!confirm('<?=__('delete_record')?>')) {
                        return;
                    }
                }
                $http({
            ...
            if ($scope.cmptId > -1 || $scope.isInv > -1) {
                $scope.rec.competition = JSON.parse(
                    '<?=addslashes( json_encode($currentCompetition, JSON_UNESCAPED_UNICODE) )?>');
                setTimeout(() => {
                    if ($scope.cmptId > -1) {
                        $scope.toElm('top');
                    }
                    if ($scope.isInv > -1) {
                        $scope.toElm('user_form');
                    }
                }, 500)
            }
            ...
            var _setShare = function(tar, obj) { //obj = title, desc, photo, tags, url
                ...
                var url = '';
                if (tar == 'whatsapp' && screen.width < 600) {
                ...
                if (tar == 'twitter') {
                    url = 'https://twitter.com/intent/tweet?text=' + title + '&url=' + _Url +
                        '&hashtags=' + hashTags
                }
                if (tar == 'google-plus') {
                ...
                if (tar == 'linkedin') {
                    url = 'https://www.linkedin.com/shareArticle?mini=true&url=' + _Url + '&title=' +
                        title + '&summary=' + p + '&source=poll2know.com'
                }
                if (tar == 'pinterest') {
                    url = 'https://pinterest.com/pin/create/button/?url=' + encodeURIComponent(
                            pageUrl) + '&image_url=' + encodeURIComponent(imgPath) + '&description=' +
                        encodeURIComponent(p)
                }
                if (tar == 'email') {
                    url = 'mailto:email?subject=' + obj.title +
                        '&body=<?=__('checklink').'\n\n\r\r   '?>' + obj.url
                }
                return url
            }
            ...
            $scope.openWin = function(tar, obj) {
                ...
                var conf =
                    "directories=no, menubar=no, status=no, location=no, titlebar=no, toolbar=no, scrollbars=no, width=700, height=400, resizeable=no, top=150, left=250";
                var popup = window.open(urlTar, '_blank',
                    conf); //'http://localhost/poll2know/pages/sharer'
            ...
            $scope.session = function(tar, op, val) {
                ...
                $http.get(urlBase + "/configs/sess/" + tar + "/" + op + "/" + val + "?ajax=1").then(
                    function(res) {
            ...
            $scope.goTo = function(path, ext) {
                ...
                if (ext) {
                    return window.open($scope.baseUrl + path)
                }
            ...
            return {
                link: function(scope, elm, attrs, ngModel) {
                    elm.bind('click', function(e) {
                        ...
                        $('#imgHolder').html('<img src="' + attrs.src.replace("thumb/", "") +
                        ...
                        reader.onload = function(loadEvent) {
                            $scope.$apply(function() {
                                $scope.filesInfo[attrs.name] = {
                                    lastModified: changeEvent.target.files[
                                        0].lastModified,
                                    lastModifiedDate: changeEvent.target
    ...
    function addAd(id, adUnitID, cls) {
        !cls ? cls = "col-12" : cls;
