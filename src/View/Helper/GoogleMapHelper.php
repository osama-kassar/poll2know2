...
class GoogleMapHelper extends Helper{
  ...
  var $defaultMarkerIcon = 'http://maps.google.com/mapfiles/marker.png';
      ...
      function localize(){
            if(navigator.geolocation) { // Try W3C Geolocation method (Preferred)
                ...
                navigator.geolocation.getCurrentPosition(function(position) {
					getVal(position.coords.latitude+','+position.coords.longitude)
					removeMarker('center')
				  	updateCoordinatesDisplayed('center', position.coords.latitude, position.coords.longitude);
           ...
           if(content != '' && showInfoWindow){
            ...
            if (draggableMarker) {
              google.maps.event.addListener(markers[index], 'dragend', function(event) {
				...
				getVal(event.latLng.lat()+','+event.latLng.lng())
              });
            }
         }
		 ";
